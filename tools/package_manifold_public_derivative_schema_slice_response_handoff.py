#!/usr/bin/env python3
"""Generate a sidecar-owned handoff package for a future Manifold public derivative schema slice response."""
...
PACKAGE_SCHEMA = "rusty.quest.sidecar.manifold_public_derivative_schema_slice_response_handoff_package.v1"
...
def build_package(preflight_path: Path, repo_root: Path, now: str) -> dict[str, Any]:
    ...
    requirements = preflight.get("manifold_repo_slice_response_requirements", {})
    source_preflight = {
    ...
    handoff_manifest = {
        "manifest_id": "manifest.manifold_public_derivative_schema_slice_response_handoff.synthetic.001",
        "status": "candidate",
        "target_repo": "rusty.manifold",
    ...
    validation_evidence = {
        ...
        "future_manifold_gate": "manifold_repo_owns_public_derivative_schema_slice_response_handoff_acceptance_and_implementation",
    ...
    checks = [
        check(
            "public_derivative_schema_slice_response_handoff.source_preflight_ready",
            preflight_result.ok
            and preflight.get("schema") == PREFLIGHT_SCHEMA
            and preflight.get("preflight_status") == "ready_for_manifold_public_derivative_schema_slice_response_planning"
            and preflight.get("next_gate") == "manifold_public_derivative_schema_slice_response_handoff_or_operator_decision",
            {
                "schema": preflight.get("schema"),
                "preflight_status": preflight.get("preflight_status"),
                "next_gate": preflight.get("next_gate"),
        ...
        check(
            "public_derivative_schema_slice_response_handoff.no_repo_response_route_state_or_hostess",
            package_scope["repo_touch_status"] == "not_touched"
            and package_scope["branch_status"] == "not_created"
            and package_scope["implementation_plan_status"] == "not_created"
            ...
            and package_scope["hostess_input_status"] == "not_created"
            ...
            "package does not touch Manifold, create a branch, implementation plan, response, decision, schema, route, state, audit, validation report, public derivative, Hostess route/input, or live evidence",
        ),
        check(
            "public_derivative_schema_slice_response_handoff.manifold_authority",
        ...
        check(
            "public_derivative_schema_slice_response_handoff.source_chain_complete",
        ...
        check(
            "public_derivative_schema_slice_response_handoff.downstream_artifacts_from_preflight",
            ...
            and handoff_manifest["downstream_implementation_plan_status"] == "not_created"
            and handoff_manifest["downstream_implementation_status"] == "not_created"
            and handoff_manifest["downstream_response_status"] == "not_created"
            and handoff_manifest["downstream_decision_status"] == "not_decided"
            ...
            and handoff_manifest["downstream_hostess_boundary_status"] == "not_created",
            {
                ...
                "downstream_implementation_plan_status": handoff_manifest["downstream_implementation_plan_status"],
                "downstream_implementation_status": handoff_manifest["downstream_implementation_status"],
                "downstream_response_status": handoff_manifest["downstream_response_status"],
                "downstream_decision_status": handoff_manifest["downstream_decision_status"],
                ...
                "downstream_accepted_state_status": handoff_manifest["downstream_accepted_state_status"],
                "downstream_audit_status": handoff_manifest["downstream_audit_status"],
        ...
        check(
            "public_derivative_schema_slice_response_handoff.validation_revision_rejection_audit_and_route_boundaries",
            ...
            and required_audit_terms <= set(handoff_manifest["required_downstream_audit_terms"])
            ...
            and route_boundaries.get("accepted_state_owner") == "rusty.manifold"
            and handoff_manifest["rollback_policy"] == "manifold_owned_disable_route_or_reject_source",
            {
                ...
                "audit_terms": handoff_manifest["required_downstream_audit_terms"],
            ...
            },
            {
                ...
                "audit_terms": sorted(required_audit_terms),
                "route_boundaries": {
            ...
            "handoff package carries Manifold validation, revision, rejection, audit, route, and rollback constraints forward",
        ),
        check(
            "public_derivative_schema_slice_response_handoff.hostess_deferred",
        ...
        check(
            "public_derivative_schema_slice_response_handoff.privacy_and_validation_boundary",
            ...
            and validation_evidence["future_manifold_gate"] == "manifold_repo_owns_public_derivative_schema_slice_response_handoff_acceptance_and_implementation"
    ...
    return {
        ...
        "package_id": "package.manifold_public_derivative_schema_slice_response_handoff.synthetic.001",
        "generated_at": now,
        "package_status": "public_derivative_schema_slice_response_handoff_package_ready" if fail_count == 0 else "blocked",
        "source_manifold_public_derivative_schema_slice_response_implementation_preflight": source_preflight,
        ...
        "authority_boundary": [
            "The Manifold public derivative schema slice response handoff package is descriptor evidence only; it does not touch the Manifold repo, create a branch, create an implementation plan, create a response, decide the review, create schemas, create routes, create accepted state, create audit records, create validation reports, create public derivative artifacts, touch Hostess, create Hostess input, start live Quest work, use ADB, open sockets, select endpoints, install, launch, recover, copy files, or execute commands.",
            "Manifold remains the future handoff acceptance, implementation plan, response, decision, schema, route implementation, request acceptance, command/session/audit, revision, lease, rollback, validation report, and accepted-state authority.",
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--preflight", required=True, help="Generated Manifold public derivative schema slice response implementation preflight fixture.")
    parser.add_argument("--repo-root", default=".", help="Repository root.")
    parser.add_argument("--now", required=True, help="Deterministic generated_at timestamp.")
    parser.add_argument("--output", required=True, help="Output public derivative schema slice response handoff package path.")
    ...
    try:
        ...
        print(f"package_manifold_public_derivative_schema_slice_response_handoff failed: {exc}", file=sys.stderr)
        return 1
