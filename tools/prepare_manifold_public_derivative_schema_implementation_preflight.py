#!/usr/bin/env python3
"""Generate a sidecar-owned preflight for a future Manifold public derivative schema slice."""
...
def build_preflight(response_expectation_path: Path, repo_root: Path, now: str) -> dict[str, Any]:
    ...
    manifold_repo_slice_requirements = {
        "slice_class": "manifold_owned_public_derivative_schema_slice",
        "target_repo": "rusty.manifold",
        "implementation_status": "not_created_by_sidecar",
        "required_manifold_owned_artifacts": [
            {
                "artifact_id": "artifact.public_derivative_response_schema",
            ...
            {
                "artifact_id": "artifact.public_derivative_input_schema",
                "artifact_kind": "input_schema",
                "owner": "rusty.manifold",
                "status": "not_created_by_sidecar",
                "evidence_source": "proposed_manifold_schema.required_fields",
            },
            {
                "artifact_id": "artifact.public_derivative_intake_route_handler",
                "artifact_kind": "route_handler",
                "owner": "rusty.manifold",
                "status": "not_created_by_sidecar",
                "evidence_source": "proposed_manifold_route",
            },
            {
                "artifact_id": "artifact.public_derivative_decision_event_fixture",
            ...
            {
                "artifact_id": "artifact.public_derivative_accepted_state_fixture",
            ...
            {
                "artifact_id": "artifact.public_derivative_audit_fixture",
            ...
            {
                "artifact_id": "artifact.public_derivative_validation_report_fixture",
                "artifact_kind": "validation_report_fixture",
                "owner": "rusty.manifold",
                "status": "not_created_by_sidecar",
                "evidence_source": "expected_manifold_response.validation_report_ref",
            },
            {
                "artifact_id": "artifact.public_derivative_hostess_boundary_descriptor",
    ...
    validation_evidence = {
        ...
        "future_manifold_gate": "manifold_repo_owns_public_derivative_schema_slice_implementation_and_audit",
    ...
    required_rejection_terms = set(manifold_repo_slice_requirements["required_rejection_terms"])
    required_revision_terms = set(manifold_repo_slice_requirements["required_revision_terms"])

    checks = [
        check(
            "public_derivative_schema_preflight.source_expectation_ready",
            expectation_result.ok
            and response_expectation.get("schema") == EXPECTATION_SCHEMA
            and response_expectation.get("expectation_status") == "ready_for_manifold_public_derivative_schema_response"
            and response_expectation.get("next_gate") == "manifold_public_derivative_schema_response_or_operator_decision",
            {
                "schema": response_expectation.get("schema"),
                "expectation_status": response_expectation.get("expectation_status"),
                "next_gate": response_expectation.get("next_gate"),
        ...
        check(
            "public_derivative_schema_preflight.no_repo_schema_route_or_state",
            ...
            "preflight does not touch Manifold, create branch, response, decision, schema, route, state, audit, validation report, public derivative, or Hostess route",
        ),
        check(
            "public_derivative_schema_preflight.manifold_authority",
            ...
            "Manifold remains implementation plan, response, decision, schema, route, runtime, session, audit, state, and rollback owner",
        ),
        check(
            "public_derivative_schema_preflight.artifacts_manifold_owned_not_created",
            bool(artifacts)
            ...
            "all required public derivative schema artifacts are Manifold-owned and not created by the sidecar repo",
        ),
        check(
            "public_derivative_schema_preflight.validation_rejection_and_revision_terms",
            required_validation_slots <= set(manifold_repo_slice_requirements["required_validation_slots"])
            ...
            and required_revision_terms <= set(manifold_repo_slice_requirements["required_revision_terms"]),
            {
                ...
                "required_revision_terms": manifold_repo_slice_requirements["required_revision_terms"],
            },
            {
            ...
            "future Manifold slice must include fixture, route, audit, Hostess, privacy, decision, rejection, and revision coverage",
        ),
        check(
            "public_derivative_schema_preflight.route_boundaries",
            ...
            "future Manifold route preflight remains low-rate sanitized summary input only and rejects sidecar device-action drift",
        ),
        check(
            "public_derivative_schema_preflight.hostess_deferred",
        ...
        check(
            "public_derivative_schema_preflight.privacy_and_validation_boundary",
            ...
            and validation_evidence["future_manifold_gate"] == "manifold_repo_owns_public_derivative_schema_slice_implementation_and_audit"
    ...
    return {
        ...
        "preflight_id": "preflight.manifold_public_derivative_schema_implementation.synthetic.001",
        "generated_at": now,
        "preflight_status": "ready_for_manifold_public_derivative_schema_slice_planning" if fail_count == 0 else "blocked",
        "source_manifold_public_derivative_schema_response_expectation": source_response_expectation,
        ...
        "authority_boundary": [
            "The Manifold public derivative schema implementation preflight is descriptor evidence only; it does not touch the Manifold repo, create a branch, create a response, decide the review, create schemas, create routes, create accepted state, create audit records, create validation reports, create public derivative artifacts, touch Hostess, start live Quest work, use ADB, open sockets, select endpoints, install, launch, recover, copy files, or execute commands.",
            "Manifold remains the future implementation plan, response, decision, schema, route implementation, request acceptance, command/session/audit, revision, lease, rollback, and accepted-state authority.",
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--response-expectation", required=True, help="Manifold public derivative schema response expectation fixture.")
    ...
    try:
        ...
        print(f"prepare_manifold_public_derivative_schema_implementation_preflight failed: {exc}", file=sys.stderr)
        return 1
