#!/usr/bin/env python3
"""Generate the expected envelope for a future Manifold-owned public derivative schema response."""
...
EXPECTATION_SCHEMA = "rusty.quest.sidecar.manifold_public_derivative_schema_response_expectation.v1"
...
def build_expectation(schema_request_path: Path, repo_root: Path, now: str) -> dict[str, Any]:
    schema_request = load_json(schema_request_path)
    request_result = validate_repo.validate_json_file(schema_request_path)

    source_request = {
        "path": relative_output_path(schema_request_path, repo_root),
        "schema": schema_request.get("schema"),
        "request_id": schema_request.get("request_id"),
        "request_status": schema_request.get("request_status"),
        "next_gate": schema_request.get("next_gate"),
    }
    ...
    expected_manifold_response = {
        "response_class": "manifold_owned_public_derivative_schema_review_response",
        "response_status": "not_created",
        "decision_status": "not_decided",
        "allowed_decisions": [
    ...
    validation_evidence = {
        ...
        "future_manifold_gate": "manifold_repo_owns_public_derivative_schema_response_route_state_and_audit",
    ...
    required_decisions = set(expected_manifold_response["allowed_decisions"])
    required_fields = set(expected_manifold_response["required_fields"])
    required_rejections = set(expected_manifold_response["required_rejection_terms"])
    required_revisions = set(expected_manifold_response["required_revision_terms"])
    required_audit_terms = set(expected_manifold_response["required_audit_terms"])
    disallowed_content = set(expected_manifold_response["disallowed_response_content"])

    checks = [
        check(
            "public_derivative_schema_response_expectation.source_request_ready",
            request_result.ok
            and schema_request.get("schema") == REQUEST_SCHEMA
            and schema_request.get("request_status") == "ready_for_manifold_public_derivative_schema_review"
            and schema_request.get("next_gate") == "manifold_repo_public_derivative_schema_review_or_operator_decision",
            {
                "schema": schema_request.get("schema"),
                "request_status": schema_request.get("request_status"),
                "next_gate": schema_request.get("next_gate"),
            ...
            relative_output_path(schema_request_path, repo_root),
        ),
        check(
            "public_derivative_schema_response_expectation.no_response_schema_route_or_state",
            ...
            "response expectation is descriptor-only and does not mutate Manifold, Hostess, or runtime state",
        ),
        check(
            "public_derivative_schema_response_expectation.authority_split",
            ...
            "Manifold owns future response, decision, schema, route, state, audit, and rollback; sidecar remains observer/proposer",
        ),
        check(
            "public_derivative_schema_response_expectation.response_contract",
            ...
            and required_decisions <= set(expected_manifold_response["allowed_decisions"])
            and required_fields <= set(expected_manifold_response["required_fields"])
            and required_rejections <= set(expected_manifold_response["required_rejection_terms"])
            and required_revisions <= set(expected_manifold_response["required_revision_terms"])
            and required_audit_terms <= set(expected_manifold_response["required_audit_terms"])
            and disallowed_content <= set(expected_manifold_response["disallowed_response_content"])
            and expected_manifold_response["accepted_state_policy"] == "manifold_owned_monotonic_revision"
            and expected_manifold_response["rollback_policy"] == "manifold_owned_disable_route_or_reject_source",
            expected_manifold_response,
            {
                "response_status": "not_created",
                "decision_status": "not_decided",
            ...
            "future Manifold response defines bounded accept/revision/reject semantics without creating state",
        ),
        check(
            "public_derivative_schema_response_expectation.no_public_derivative_or_hostess_input",
            expected_manifold_response["public_derivative_policy"] == "response_does_not_create_derivative_artifact"
            and expected_manifold_response["hostess_input_policy"] == "response_does_not_create_hostess_input"
            and response_expectation_scope["public_derivative_status"] == "not_created"
            and response_expectation_scope["operator_approval_status"] == "not_recorded"
            and response_expectation_scope["hostess_route_status"] == "not_created",
            {
                "public_derivative_policy": expected_manifold_response["public_derivative_policy"],
                "hostess_input_policy": expected_manifold_response["hostess_input_policy"],
            ...
            "future response expectation does not produce derivative evidence or direct Hostess input",
        ),
        check(
            "public_derivative_schema_response_expectation.hostess_deferred",
        ...
        check(
            "public_derivative_schema_response_expectation.privacy_and_validation_boundary",
            ...
            and validation_evidence["future_manifold_gate"] == "manifold_repo_owns_public_derivative_schema_response_route_state_and_audit"
    ...
    return {
        ...
        "expectation_id": "expectation.manifold_public_derivative_schema_response.synthetic.001",
        "generated_at": now,
        "expectation_status": "ready_for_manifold_public_derivative_schema_response" if fail_count == 0 else "blocked",
        "source_manifold_public_derivative_schema_request": source_request,
        ...
        "authority_boundary": [
            "The Manifold public derivative schema response expectation is descriptor evidence only; it does not touch the Manifold repo, create a branch, create a schema, create a route, create accepted state, create audit records, submit evidence, touch Hostess, start live Quest work, use ADB, select endpoints, install, launch, recover, copy files, or execute commands.",
            "The sidecar may define the expected response envelope, but Manifold remains the schema, route, decision, command/session/audit, handoff acceptance, and accepted-state authority.",
            "Operator approval and redaction review remain required before any real public derivative evidence can be accepted.",
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--schema-request", required=True, help="Generated Manifold public derivative schema request fixture.")
    parser.add_argument("--repo-root", default=".", help="Repository root.")
    parser.add_argument("--now", required=True, help="Deterministic generated_at timestamp.")
    parser.add_argument("--output", required=True, help="Output Manifold public derivative schema response expectation path.")
    args = parser.parse_args(argv)

    try:
        repo_root = Path(args.repo_root).resolve()
        expectation = build_expectation(Path(args.schema_request), repo_root, args.now)
        write_json(Path(args.output), expectation)
    except (OSError, ValueError, KeyError, json.JSONDecodeError) as exc:
        print(f"prepare_manifold_public_derivative_schema_response_expectation failed: {exc}", file=sys.stderr)
        return 1
